...
//    int racketYPosition;  // top left corner of the racket
    ...
    public GameEngine(Context context, int w, int h) {
        ...
        this.ball = new GameBall(this.screenWidth / 2, this.screenHeight / 2 - 400);

        this.racket = new GameRacket(350, 1200);
...
//            this.ballYPosition = this.ballYPosition - 25;
            this.ball.setBallY(this.ball.getBallY());
            // if ball hits ceiling, then change directions
            if (this.ball.getBallY() <= 0 ) {
                // hit upper wall
...
//            this.racketXPosition = this.racketXPosition + 10;
            this.racket.setRacketX(this.racket.getRacketX()+10);
        }
...
//                (ballYPosition + 50) >= (this.racketYPosition) &&
                (this.ball.getBallY() ) >= (this.racket.getRacketX()) &&
                this.ball.getBallX() >= (this.racket.getRacketX() )&&
                this.ball.getBallX()<= this.racket.getRacketX()+ 400
        ) {
    ...
    public void redrawSprites() {
        if (this.holder.getSurface().isValid()) {
            ...
            this.canvas.drawRect(this.racket.racketX,
                    this.racket.racketY,
                    this.racket.racketX + 400,     // 400 is width of racket
                    this.racket.racketY + 50,    // 50 is height of racket
                    paintbrush);
            paintbrush.setColor(Color.WHITE);
            ...
            paintbrush.setTextSize(60);
            canvas.drawText("Remaining Lives:-" + this.Lives,25,200,paintbrush );
            //----------------
